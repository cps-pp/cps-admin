...
import { UseFormRegister, FieldErrors, RegisterOptions, UseFormGetValues, Controller, Control } from "react-hook-form";
...
const PriceInput: FC<PriceInputProps> = ({
  ...
  const registerMethod = register
    ? register(name, {
        ...formOptions,
        value: defaultValue, // Explicitly set the value
        onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
          const rawValue = e.target.value.replace(/[^0-9]/g, '');
          const formattedValue = formatNumber(rawValue);
          setDisplayValue(formattedValue);

          formOptions?.onChange?.(e as any);
        },
  ...
  return (
    <div>
      ...
      <div className="relative">
        {control ? (
          // Use Controller for better control with react-hook-form when control is provided
          <Controller
            name={name}
            control={control}
            ...
            render={({ field }) => (
              <input
                ...
                onChange={(e) => {
                  ...
                  const formattedValue = formatNumber(rawValue);
                  setDisplayValue(formattedValue);
                  field.onChange(rawValue); // Pass raw numeric value to form
                }}
                onBlur={field.onBlur}
                ref={field.ref}
              />
          ...
          // Use register method when no control is provided
          <input
            className={`relative z-20 w-full mb-2 appearance-none rounded border border-stroke bg-transparent py-3 px-4.5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:disabled:bg-meta-4 dark:focus:border-primary text-black dark:text-white capitalize
                ${errors?.[name] ? 'border-red-500' : ''} ${className}`}
            type="text"
