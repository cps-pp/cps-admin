...
import { ArrowRight, HandCoins, HandCoinsIcon, PieChart } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const response = await fetch(`${URLBaseLocal}/src/report/payment`);
  ...
  const handleSubmitExchangeRates = async (rates) => {
    ...
    try {
      const res = await fetch(`${URLBaseLocal}/src/manager/update-rates`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${URLBaseLocal}/src/manager/patient`);
        if (!response.ok)
          throw new Error(`HTTP error! Status: ${response.status}`);
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchMedicines = async () => {
      try {
        ...
        const response = await fetch(`${URLBaseLocal}/src/manager/medicines`);
        if (!response.ok)
          throw new Error(`HTTP error! Status: ${response.status}`);
        const data = await response.json();
        ...
        if (data && data.data) {
          ...
          // เพิ่มการประมวลผลข้อมูลการใช้ยา
          processMedicineUsage(data.data);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchMedicineTypes = async () => {
      try {
        const response = await fetch(`${URLBaseLocal}/src/manager/category`);
        if (!response.ok)
          throw new Error(`HTTP error! Status: ${response.status}`);
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const response = await fetch(`${URLBaseLocal}/src/appoint/appointment`);
        const data = await response.json();
  ...
  const [medicineUsageLoading, setMedicineUsageLoading] = useState(true);

  const processMedicineUsage = (medicineData) => {
    try {
      setMedicineUsageLoading(true);

      const usageData = medicineData
        .filter((med) => med.qty !== undefined && med.qty !== null)
        .map((med) => {
          const currentQty = parseInt(med.qty) || 0;
          const initialStock = 100;
          const usageCount = Math.max(0, initialStock - currentQty);
          ...
          return {
            ...
            color: COLORS[0], // จะถูกกำหนดใหม่ด้านล่าง
          };
        })
        .filter((item) => item.usage_count > 0) // เอาเฉพาะยาที่มีการใช้
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-300 rounded shadow-lg">
          <p className="font-medium">{data.med_name}</p>
          <p className="text-sm text-gray-600">
            ຈຳນວນການໃຊ້: {data.usage_count} ຄັ້ງ
          </p>
          <p className="text-sm text-gray-600">
            ເປີເຊັນ:{' '}
            {(
              (data.usage_count /
                medicineUsage.reduce(
      ...
      <div className="grid grid-cols-2 gap-8 mt-6">
        ...
        {/* <div className="rounded bg-white p-6 shadow-md">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-md md:text-lg lg:text-xl font-medium text-strokedark">
              ຢາທີ່ມີການໃຊ້ສູງ (ຄາດການ)
            </h2>
            <HandCoins className="w-6 h-6 text-Third2" />
          </div>
          
          {medicineUsageLoading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    labelLine={false}
                    label={({ med_name, usage_count }) => `${med_name}: ${usage_count}`}
                    outerRadius={80}
                    fill="#8884d8"
                    dataKey="usage_count"
                  >
            ...
            <div className="flex justify-center items-center h-64">
              <div className="text-gray-500">ບໍ່ມີຂໍ້ມູນການໃຊ້ຢາ</div>
            </div>
          )}
          
          {!medicineUsageLoading && medicineUsage.length > 0 && (
            <div className="mt-4 space-y-2">
              <h3 className="text-sm font-medium text-gray-700">ລາຍລະອຽດ (ຄາດການຈາກ Stock):</h3>
              {medicineUsage.map((item, index) => (
                <div key={index} className="flex items-center justify-between text-sm">
                  ...
                  <div className="text-right">
                    <div className="font-medium">ໃຊ້: {item.usage_count}</div>
                    <div className="text-xs text-gray-500">ເຫຼືອ: {item.current_qty}</div>
                  </div>
                </div>
              ))}
            </div>
