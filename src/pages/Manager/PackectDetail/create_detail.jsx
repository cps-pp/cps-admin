...
const AddDetailPacket = ({ id, setShow, getList, onClose }) => {
    ...
    useEffect(() => {
        const fetchPacketDetails = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:4000/src/manager/packet-detail/${id}`);
                if (!response.ok) {
                    throw new Error(`HTTP error! Status: ${response.status}`);
                }
                const data = await response.json();
    ...
    const checkMedicineExists = async (ser_id, med_id) => {
        try {
            const response = await fetch(`http://localhost:4000/src/manager/check-medicine-in-packet/${ser_id}/${med_id}`);
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            }
            const data = await response.json();
    ...
    const updateMedicineQuantity = async (packetdetail_id, additionalQty) => {
        try {
            const response = await fetch(`http://localhost:4000/src/manager/packet-detail/${packetdetail_id}`, {
                method: 'PUT',
    ...
    const addNewMedicine = async (formData) => {
        try {
            ...
            const response = await fetch('http://localhost:4000/src/manager/packet-detail', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleDeleteClick = (packetDetailId) => {
        ...
        const itemIndex = packetDetails.findIndex(detail => detail.packetdetail_id === packetDetailId);
    ...
    const handleDeleteDetail = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:4000/src/manager/packet-detail/${deleteDetailId}`, {
                method: 'DELETE',
                headers: {
    ...
    const fetchPacketDetailsAgain = async () => {
        ...
        try {
            const detailResponse = await fetch(`http://localhost:4000/src/manager/packet-detail/${id}`);
        ...
        <div className="rounded bg-white pt-4 dark:bg-strokedark">
            <div className="flex items-center justify-between border-b border-stroke dark:border-strokedark pb-4">
                <h1 className="text-md md:text-lg lg:text-xl font-medium text-strokedark dark:text-bodydark3 px-4">
                    ເພີ່ມລາຍລະອຽດແພ็ກເກັດ - ຊຸດຢາ: {id}
                </h1>
            </div>

            {/* แสดงข้อมูลพื้นฐานของชุดยา */}
            <div className="px-4 py-2 bg-gray-50 dark:bg-gray-800">
                <p className="text-sm text-gray-600 dark:text-gray-300">
                    <strong>ລະຫັດຊຸດຢາ:</strong> {id}
                    <span className="ml-4"><strong>ສະຖານະ:</strong> ກຳລັງເພີ່ມລາຍລະອຽດຢາໃນຊຸດ</span>
                </p>
            </div>

            {/* แสดงรายการยาที่มีอยู่แล้วในแพ็คเก็ต */}
            {packetDetails.length > 0 && (
                <div className="mt-4 px-4">
                    <h3 className="text-lg font-medium mb-2">ລາຍການຢາທີ່ມີຢູ່ແລ້ວໃນຊຸດ:</h3>
                    <div className="overflow-x-auto">
                        <table className="w-full border-collapse border border-gray-300">
                            <thead>
                                <tr className="bg-gray-100">
                                    <th className="border border-gray-300 px-4 py-2 text-left">ລຳດັບ</th>
                            ...
                            <tbody>
                                {packetDetails.map((detail, index) => (
                                    <tr key={`${detail.packetdetail_id}-${index}`} className="hover:bg-gray-50">
                                        ...
                                        <td className="border border-gray-300 px-4 py-2 text-center">
                                            <button
                                                onClick={() => handleDeleteClick(detail.packetdetail_id)}
            ...
            {/* ฟอร์มเพิ่มยาใหม่ในแพ็คเก็ต */}
            <form onSubmit={handleSubmit(handleSave)} className="mt-6 px-4">
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg mb-6">
                    <h3 className="text-lg font-medium mb-2 text-green-800 dark:text-green-200">ເພີ່ມຢາໃໝ່ໃນຊຸດ:</h3>
                </div>
            ...
            {/* ConfirmModal สำหรับยืนยันการเพิ่มยาที่มีอยู่แล้ว */}
            <ConfirmModal
