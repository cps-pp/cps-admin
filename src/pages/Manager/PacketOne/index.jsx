...
import CreatePacket from './crate';
  ...
  const handleDeletePacket = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/src/manager/packet/${selectedPacketId}`,
        {
          method: 'DELETE',
        },
      );
              ...
              {paginatedPackets.length > 0 ? (
                paginatedPackets.map((packet, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">{packet.packet_name}</td>
                    <td className="px-4 py-4">
                      {packet.price.toLocaleString()}
                    </td>
