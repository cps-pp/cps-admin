...
useEffect(() => {
  if (inspectionData) {
    console.log('โหลด inspectionData:', inspectionData);

    if (inspectionData.patient_id) {
      setSelectedPatient({
        patient_id: inspectionData.patient_id,
        patient_name: inspectionData.patient_name,
        patient_surname: inspectionData.patient_surname,
        gender: inspectionData.gender,
      });
    }

    const formattedDate = inspectionData.date
      ? new Date(inspectionData.date).toISOString().split('T')[0]
      : '';
    ...
    setIntivalue((prev) => ({
      ...prev,
      symptom: inspectionData.symptom || '',
      checkup: inspectionData.checkup || '',
      diseases_now: inspectionData.diseases_now || '',
      note: inspectionData.note || '',
    }));

    if (inspectionData.in_id) {
      setInspectionId(inspectionData.in_id);
    }
  }
}, [inspectionData]);
  ...
  return (
    <div className="">
      ...
      <div className="overflow-x-auto mb-4">
        <TypeService refreshKey={refreshKey} inspectionId={inspectionId} />
      </div>
