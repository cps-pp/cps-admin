...
const SelectPatientPopup = ({ patients, onClose, onSelect }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // ส่งข้อมูลคนเจ็บกลับไปยัง parent component
      if (onSelect) {
  ...
  return (
    <div className={`fixed inset-0 bg-black transition-opacity duration-300 ease-in-out flex items-center justify-center z-50 ${isVisible ? 'bg-opacity-50' : 'bg-opacity-0'}`}>
      <div className={`bg-white p-4 rounded shadow-xl w-full md:max-w-lg lg:max-w-xl relative overflow-auto h-[600px] max-h-[100vh] transition-all duration-300 transform ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
        ...
        <div className="border border-purple-100 rounded overflow-hidden mb-4">
          <ul className="h-[450px] overflow-y-auto divide-y divide-purple-100">
            {[...patients]
              .sort((a, b) => (a.patient_id === 'PT0' ? -1 : b.patient_id === 'PT0' ? 1 : 0))
              .map((p) => (
                <li
                  key={p.patient_id}
                ...
                >
                  <div className="font-medium">
                    {p.patient_id} {p.patient_name} {p.patient_surname} {p.gender}
                  </div>
                </li>
              ))}
          </ul>
