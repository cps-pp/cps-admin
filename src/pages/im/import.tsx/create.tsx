...
const CreateImport: React.FC<CreateProps> = ({ setShow, getList }) => {
  const {
    register,
    handleSubmit,
    reset,
    ...
    const fetchEmp = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const transformedData = data.data.map((emp: any) => ({
            id: emp.emp_id,
            name: emp.emp_name,
 ...
 const handleSave = async (data: any) => {
  ...
  try {
    ...
    formData.append('lot', data.lot || ''); // Add lot field, even if empty
