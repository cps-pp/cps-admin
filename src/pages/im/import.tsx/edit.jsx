...
<<<<<<< HEAD:src/pages/im/import.tsx/edit.jsx
=======
import DatePicker from '@/components/DatePicker_two';
>>>>>>> 04c0b8aa93908363f1af5f8ef9006db261d3577b:src/pages/im/import.tsx/edit.tsx
...
const EditImport = ({ setShow, getList, id, onClose }) => {
  ...
  useEffect(() => {
    const handleBeforeUnload = (event) => {
      if (isDirty) {
        const message = 'ທ່ານຍັງບໍ່ໄດ້ບັນທືກຂໍ້ມູນ ຢືນຢັນວ່າຈະອອກຈາກໜ້ານີ້ແທ້ບໍ່?';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (medRes.ok) {
          setMedicine(medData.data.map((m) => ({
            med_id: m.med_id,
            med_name: m.med_name,
          })));
        }
        ...
        if (supRes.ok) {
          setSupplier(supData.data.map((s) => ({
            sup_id: s.sup_id,
        ...
        if (empRes.ok) {
          setEmployees(empData.data.map((emp) => ({
            id: emp.emp_id,
            name: emp.emp_name,
  ...
  const handleSave = async (data) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4000/src/im/import/${id}`, {
        method: 'PUT',
        body: formData,
      });
