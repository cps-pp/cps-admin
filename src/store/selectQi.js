...
const useStoreQi = create((set) => ({
  ...
  fetchInspectionEquipmentById: async (id) => {
      ...
      try {
        const res = await fetch(`${URLBaseLocal}/src/report/prescription?id=${id}&med_type=M2`);
  ...
  addEquipmentNews: (data) => {
    set((state) => {
      const existsID = state.newEquipment.some(item => item.med_id === data.med_id);
  ...
  removeEquipmentNews: (data) => {
    set((state) => {
      const filteredData = state.newEquipment.filter((item) => item.med_id !== data.med_id);
      return {
        ...state,
